...
class SpringSystem {
  ...
  void update(float dt) {
    ...
    for (int i=0;i<velocity.length;i++) {
     ...
     //for spatial diagonal strings one 
     for (int i=0;i<velocity.length-1;i++) {
      for (int j=0;j<velocity[0].length-1;j++) {
        PVector e = PVector.sub(position[i+1][j+1],position[i][j]);
        float l = e.mag();
        e = PVector.div(e,l);
        float v1 = PVector.dot(e,velocity[i][j]);
        float v2 = PVector.dot(e,velocity[i+1][j+1]);
        float f = -ks*(1.41*loose-l) -kd*(v1-v2);
        vn[i][j] = PVector.add(vn[i][j],PVector.mult(e,f*dt));
        vn[i+1][j+1] = PVector.sub(vn[i+1][j+1],PVector.mult(e,f*dt));
      }
    }
    
     //for spatial diagonal strings two
     for (int i=0;i<velocity.length-1;i++) {
      for (int j=0;j<velocity[0].length-1;j++) {
        PVector e = PVector.sub(position[i+1][j],position[i][j+1]);
        float l = e.mag();
        e = PVector.div(e,l);
        float v1 = PVector.dot(e,velocity[i][j+1]);
        float v2 = PVector.dot(e,velocity[i+1][j]);
        float f = -ks*(1.41*loose-l) -kd*(v1-v2);
        vn[i][j+1] = PVector.add(vn[i][j+1],PVector.mult(e,f*dt));
        vn[i+1][j] = PVector.sub(vn[i+1][j],PVector.mult(e,f*dt));
      }
    }
    ...
    //mid_point method
    PVector[][] mid_point_v=vn;
    for (int i=0;i<velocity.length;i++) {
      for (int j=0;j<velocity[0].length;j++) {
        PVector mid_point_velocity = PVector.div( (PVector.add(vn[i][j],velocity[i][j])),2.0);
        mid_point_v[i][j] = mid_point_velocity;
      }
    }
    ...
    for (int i=0;i<velocity.length;i++) {
      ...
      mid_point_v[i][0] = new PVector(0,0,0);
    }
      
    // update position
    for (int i=0;i<velocity.length;i++) {
     for (int j=0;j<velocity[0].length;j++) {
       position[i][j]=PVector.add(position[i][j],PVector.mult(mid_point_v[i][j],dt));
      }
    }
